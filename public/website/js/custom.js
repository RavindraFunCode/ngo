...
(function ($) {
	...
	*/
	...
	if ($('#donate-popup').length) {
		...
		// Dynamic Currency and Validation Logic
		var $countrySelect = $('#donation-country');
		var $currencyInput = $('#donation-currency');
		var $currencyLabels = $('.chicklet-list li label');
		var $customAmountInput = $('#other-amount');
		...
		$donateForm.on('submit', function (e) {
			var $selectedOption = $countrySelect.find('option:selected');
			var minPhone = $selectedOption.data('min-phone');
			var maxPhone = $selectedOption.data('max-phone');
			var phoneValue = $phoneInput.val().replace(/\D/g, ''); // Remove non-digits

			// Phone Validation
			if (phoneValue.length < minPhone || phoneValue.length > maxPhone) {
				e.preventDefault();
				alert('Phone number must be between ' + minPhone + ' and ' + maxPhone + ' digits for ' + $selectedOption.text().trim());
				return false;
			}

			// Custom Amount Validation
			var customAmount = $customAmountInput.val();
