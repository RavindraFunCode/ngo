...
class ThemeLayout {
    ...
    initSwitchListener() {
        ...
        window.addEventListener('storage', function (e) {
            if (e.key === '__LARKON_CONFIG__') {
                var newConfig = JSON.parse(e.newValue);
                if (newConfig) {
                    self.config = newConfig;
                    self.html.setAttribute('data-bs-theme', newConfig.theme);
                    self.html.setAttribute('data-topbar-color', newConfig.topbar.color);
                    self.html.setAttribute('data-menu-color', newConfig.menu.color);
                    if (window.innerWidth > 1140) {
                        self.html.setAttribute('data-menu-size', newConfig.menu.size);
                    }
                    self.setSwitchFromConfig();
                }
            }
        });
    }
