                ...
                <form id="editEventForm" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" name="start_time" class="form-control" value="{{ $event->start_time }}" placeholder="e.g. 09:30 PM">
                        </div>
                        <div class="col-md-6 mb-3">
                    ...
                    <div class="tab-content pt-3" id="langTabContent">
                        @foreach($languages as $index => $language)
                        @php
                            $translation = $event->getTranslation($language->code);
                        @endphp
                        <div class="tab-pane fade {{ $index === 0 ? 'show active' : '' }}" id="lang-{{ $language->code }}" role="tabpanel">
                            <div class="mb-3">
                                ...
                                <input type="text" name="translations[{{ $language->code }}][title]" class="form-control" value="{{ $translation->title ?? '' }}" {{ $language->is_default ? 'required' : '' }}>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Description ({{ $language->code }})</label>
                                <textarea name="translations[{{ $language->code }}][description]" class="form-control" rows="5">{{ $translation->description ?? '' }}</textarea>
                            </div>
                        </div>
                        @endforeach
                    </div>
...
<script>
    $('#editEventForm').on('submit', function(e) {
        ...
        $.ajax({
            ...
            type: 'POST', // Form spoofing handled by @method('PUT') which adds _method field
            data: formData,
            processData: false,
            contentType: false,
