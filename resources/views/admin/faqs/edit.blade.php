    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form id="editForm" action="{{ route('admin.faqs.update', $faq) }}" method="POST">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="tab-pane fade {{ $loop->first ? 'show active' : '' }}" id="lang-{{ $language->code }}" role="tabpanel">
                                <div class="mt-3">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="question_{{ $language->code }}" name="translations[{{ $language->code }}][question]" value="{{ $translation->question ?? '' }}" maxlength="255">
                                    </div>
                                    <div class="mb-3">
                                        <label for="answer_{{ $language->code }}" class="form-label">Answer</label>
                                        <textarea class="form-control" id="answer_{{ $language->code }}" name="translations[{{ $language->code }}][answer]" rows="5">{{ $translation->answer ?? '' }}</textarea>
                                    </div>
                                </div>
                            </div>
                        @endforeach
